...
# format
EXCEPTION_HANDLER = getattr(settings, 'INERTIA_EXCEPTION_HANDLER',
                            'drf_inertia.exceptions.DefaultExceptionHandler')

# The auth redirect is used in the default exception handler
# to determine where to go when 401 or 403 errors are raised
AUTH_REDIRECT = getattr(settings, 'INERTIA_AUTH_REDIRECT', '/accounts/login')

# if AUTH_REDIRECT_URL_NAME is specified use django.urls.reverse
# to get the AUTH_REDIRECT instead
AUTH_REDIRECT_URL_NAME = getattr(
    settings, 'INERTIA_AUTH_REDIRECT_URL_NAME', None)
